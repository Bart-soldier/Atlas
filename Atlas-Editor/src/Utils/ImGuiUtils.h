...
namespace Atlas
{
	...
	public:
		...
		static bool DragFloat(const std::string& label, float& value, float resetValue = 0.0f, float speed = 1.0f, float min = 0.0f, float max = 0.0f, int uniqueID = -1);
		...
		static bool BeginCombo(const std::string& label, const char& value, float maxWidth = -1, int uniqueID = -1);
