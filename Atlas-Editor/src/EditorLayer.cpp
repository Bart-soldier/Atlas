...
namespace Atlas
{
	...
	void EditorLayer::OnUpdate(Timestep ts)
	{
		...
		if (mouseX >= 0 && mouseY >= 0 && mouseX < (int)viewportSize.x && mouseY < (int)viewportSize.y)
		{
			...
			ATLAS_CORE_WARN("Pixel data = {0}", pixelData);
		}
	...
	{
		std::optional<std::string> filepath = FileDialogs::OpenFile("Atlas Scene (*.atlas)\0*.atlas\0");
		if (filepath)
		{
			m_ActiveScene = CreateRef<Scene>();
			m_ActiveScene->OnViewportResize((uint32_t)m_ViewportSize.x, (uint32_t)m_ViewportSize.y);
	...
	{
		std::optional<std::string> filepath = FileDialogs::SaveFile("Atlas Scene (*.atlas)\0*.atlas\0");
		if (filepath)
		{
			SceneSerializer serializer(m_ActiveScene);
